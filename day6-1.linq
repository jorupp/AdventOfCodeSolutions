...
void Main()
{
	...
	var instructions = @"turn on 489,959 through 759,964
...
toggle 628,958 through 811,992";
	var re = new Regex(@"(turn on|turn off|toggle) (\d*),(\d*) through (\d*),(\d*)");
	foreach(var line in instructions.Split('\n')) {
		...
		Func<bool, bool> operation = 
			m.Groups[1].Value == "turn on" ? ((Func<bool, bool>)On) :
			m.Groups[1].Value == "turn off" ? ((Func<bool, bool>)Off) :
			m.Groups[1].Value == "toggle" ? ((Func<bool, bool>)Toggle) :
				Invalid;
	...
	foreach(var x in Enumerable.Range(0, 1000)) {
		foreach(var y in Enumerable.Range(0, 1000)) {
			lightsOn += lights[x,y] ? 1 : 0;
		}
	}
	lightsOn.Dump();
}

bool On(bool i) { return true; }
bool Off(bool i) { return false; }
bool Toggle(bool i) { return !i; }
bool Invalid(bool i) { throw new ApplicationException(); }
