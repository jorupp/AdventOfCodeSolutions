...
toggle 628,958 through 811,992";
	...
	foreach(var line in instructions.Split('\n')) {
		...
		Func<int, int> operation = 
			m.Groups[1].Value == "turn on" ? ((Func<int, int>)On) :
			m.Groups[1].Value == "turn off" ? ((Func<int, int>)Off) :
			m.Groups[1].Value == "toggle" ? ((Func<int, int>)Toggle) :
				Invalid;
...
int On(int i) { return i + 1; }
int Off(int i) { return Math.Max(0, i-1); }
int Toggle(int i) { return i + 2; }
int Invalid(int i) { throw new ApplicationException(); }
